...
`esy-glpk` is package, [`glpk`](), packaged for [`esy`](https://esy.sh/).
...
`esy-glpk` can be used from both NPM and directly from Github.

### From NPM 

`esy-glpk` is deployed on NPM can be found
[here](https://www.npmjs.com/package/TODO).

You can simply run `esy add esy-glpk` to install it, or specify it in
...
{
  "dependencies": {
    "esy-glpk": "esy-packages/esy-glpk"
  }
}
```
...
```diff
  "dependencies": {
+   "esy-glpk": "esy-packages/esy-glpk#<commit hash>"
  }
```
